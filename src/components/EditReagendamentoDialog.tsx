...
interface EditReagendamentoDialogProps {
  isOpen: boolean;
  onClose: () => void;
  item: Reagendamento | null;
  onUpdated: (r: Reagendamento) => void;
  tecnicos: string[];
  produtos: string[];
  pecas: string[];
...
export default function EditReagendamentoDialog({
  isOpen,
  onClose,
  item,
  onUpdated,
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        ...
        teveReagendamento: !!item.teveReagendamento,
        motivo: String(item.motivo ?? ''),
        codigoPeca: String(item.codigoPeca ?? ''),
  tipo: String(item.tipo ?? 'FUNCIONAL'),
        nomePeca: String(item.nomePeca ?? ''),
      });
    }
  }, [item]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/reagendamentos/${item.id}` , {
      method: 'PUT',
    ...
    onUpdated(saved as Reagendamento);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white/95 text-gray-900 backdrop-blur rounded-2xl border border-gray-200/60 shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200/60">
          <h2 className="text-xl font-semibold text-gray-900">Editar Reagendamento</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">Número da OS *</label>
              <input
                type="text"
                name="os"
                value={formData.os}
                ...
                required
                className="w-full border border-gray-200 rounded-xl px-3 py-2 bg-white/70 text-gray-900 placeholder:text-gray-400 shadow-sm focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400/60"
                placeholder="Ex: 701424523"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">SKU *</label>
              <select
                name="sku"
                value={formData.sku}
                onChange={handleInputChange}
                required
                className="w-full border border-gray-200 rounded-xl px-3 py-2 bg-white/70 text-gray-900 shadow-sm focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400/60"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">Produto</label>
              <input
                type="text"
                name="produto"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">Motivo *</label>
              <select
                name="motivo"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-2">Código da Peça</label>
              <input
                type="text"
                name="codigoPeca"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-800 mb-2">Nome da Peça</label>
              <input
                type="text"
                name="nomePeca"
